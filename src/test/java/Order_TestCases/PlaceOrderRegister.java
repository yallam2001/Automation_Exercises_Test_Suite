...
public class PlaceOrderRegister {
    ...
    public Object[][] setUpClass() throws FileNotFoundException {
        RegisterCheckoutUser[] checkoutUsers = OrderHelperClass.
                ReadRegisterWhileCheckoutData(
...
//        options.addArguments("--disable-notifications");
    ...
    @Description(
            "This test case examines registering an account after placing an order works as intended or not")
    @Severity(SeverityLevel.NORMAL)
    @Owner("Yahia Allam")
    @Link(name = "Website", url = "https://automationexercise.com/")
    @Issue("CHK-001")
    public void PlaceOrderRegisterWhileCheckout(RegisterCheckoutUser checkoutUser,
            FormUser formData) {
        addProductsToCart();
        proceedToCheckoutAsGuest();
        startSignUpDuringCheckout(checkoutUser.name, checkoutUser.email);
    ...
    @Step("Proceed to checkout as guest")
    public void proceedToCheckoutAsGuest() {
        ct.clickProceedToCheckout();
        AllureUtil.takeScreenshot(Browser, "Proceeded to Checkout as Guest");
    }

    @Step("Start sign-up during checkout with Name: {name}, Email: {email}")
    public void startSignUpDuringCheckout(String name,
            String email) {
        s.enterNameField(name);
        s.enterEmailAddressField(email);
        s.clickSignUpButton();
        AllureUtil.takeScreenshot(Browser, "Sign-Up Started During Checkout");
    }
