...
//        options.addArguments("--disable-notifications");
    ...
    @Description(
            "This test case examines downloading the invoice after making an order")
    @Severity(SeverityLevel.NORMAL)
    @Owner("Yahia Allam")
    @Link(name = "Website", url = "https://automationexercise.com/")
    @Issue("DWN-001")
    public void DownloadInvoice(InvoiceUser invoiceUser,
            FormUser formData) {
        openLoginPage();
        verifyNewUserSignUpText();
        enterSignUpDetails(invoiceUser.name, invoiceUser.email);
    ...
    public void enterCardDetails(FormUser formData) {
        pay.enterCardDetails(formData.cardName, formData.cardNumber,
                formData.cvv, formData.expiryMonth, formData.expiryYear);
        AllureUtil.takeScreenshot(Browser, "Card Details Entered");
    }

    @Step("Verify order success message is displayed")
    public void verifyOrderSuccess() {
        a.assertTrue(ch.verifyOrderSuccess().toLowerCase().
                contains("your order"));
    ...
    public void downloadInvoice() {
        ch.clickDownloadInvoice();
        AllureUtil.takeScreenshot(Browser, "Invoice Downloaded");
    }
